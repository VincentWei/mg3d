...
static int MG3DWndProc(HWND hWnd, int message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch ( message ) {
        ...
        case MSG_TIMER: break; case MSG_PAINT: FillBox(mem_dc, 0, 0, SCREEN_W, SCREEN_H); switch (render_mode)
	   {
	    ...
	    {
	        mg3dTriangleF(mem_dc,  render_type[poly_info[i].type], texture, poly_info[i].vtx[0], poly_info[i].vtx[1], poly_info[i].vtx[2]);
	    }
		...
		case SCANCODE_CURSORBLOCKDOWN:
		    poly_info[poly_count-1].v3d[point_count].z = poly_info[poly_count-1].v3d[point_count].z + 6.;
                    InvalidateRect(hWnd, NULL, TRUE);
		    break;
            }
           break;
	...
	case MSG_LBUTTONDOWN:
	    ...
	    cur_x = LOWORD(lParam);
            ...
            poly_info[poly_count].v3d[point_count].u = rand()%33;
            poly_info[poly_count].v3d[point_count].v = rand()%33;
