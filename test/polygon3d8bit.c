...
static int MG3DWndProc(HWND hWnd, int message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch ( message ) {
	    ...
	    {  
            ...
            mg3dCreateLightTable(&light_table, pal, 0, 0, 0, NULL);
            color_map = &light_table;
            
            /* textures are 25% transparent (75% opaque) */
            mg3dCreateTransTable(&trans_table, pal, 128, 128, 128, NULL);
	...
	case MSG_PAINT:
            ...
            switch (render_type[render_mode])
	   {
		...
		case MG3D_POLYTYPE_PTEX_MASK_TRANS:
	        ...
	        mg3dPolygon(mem_dc,  render_type[poly_info[poly_count-1].type], texture, poly_info[poly_count-1].vc, poly_info[poly_count-1].vtx);
        ...
        case MSG_KEYDOWN:
            switch (wParam){
                case SCANCODE_ESCAPE: PostMessage(hWnd, MSG_CLOSE, 0, 0);
                    break;
	...
	case MSG_LBUTTONDOWN:
	    ...
	    cur_x = LOWORD(lParam);
            ...
            poly_info[poly_count].v3d[point_count].u = rand()%32<<16;
            poly_info[poly_count].v3d[point_count].v = rand()%32<<16;
