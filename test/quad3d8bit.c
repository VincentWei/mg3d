...
static int MG3DWndProc(HWND hWnd, int message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch ( message ) {
	    ...
	    {  
            ...
            mg3dCreateTransTable(&trans_table, pal, 0, 192, 0, print_progress);
	...
	case MSG_PAINT:
            ...
            switch (render_type[render_mode])
	   {
	    ...
	    if (poly_count == 0)break;
	        mg3dQuad(mem_dc,  render_type[poly_info[poly_count-1].type], texture, poly_info[poly_count-1].vtx[0], poly_info[poly_count-1].vtx[1], poly_info[poly_count-1].vtx[2], poly_info[poly_count-1].vtx[3]);
