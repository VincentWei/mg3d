...
static int MG3DWndProc(HWND hWnd, int message, WPARAM wParam, LPARAM lParam)
{
	...
	case MSG_PAINT:
            ...
            switch (render_mode)
	   {
	    ...
	    {
	        mg3dTriangle(mem_dc,  render_type[poly_info[i].type]/* | MG3D_POLYTYPE_ZBUF*/, texture, poly_info[i].vtx[0], poly_info[i].vtx[1], poly_info[i].vtx[2]);
	    }
		...
		case SCANCODE_CURSORBLOCKDOWN:
		    poly_info[poly_count-1].v3d[point_count].z = poly_info[poly_count-1].v3d[point_count].z + 6<<16;
                    InvalidateRect(hWnd, NULL, TRUE);
		    break;
            }
           break;
